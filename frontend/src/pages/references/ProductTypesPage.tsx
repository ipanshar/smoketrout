...
export default function ProductTypesPage() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await api.delete(`/references/product-types/${id}`);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Типы товаров</h1>
        {hasPermission('references.product_types.create') && (
          <Link to="/references/product-types/new" className="btn btn-primary flex items-center gap-2">
      ...
      <div className="card overflow-hidden">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredTypes.map((type) => (
              <tr key={type.id} className="hover:bg-gray-50">
                ...
                <td className="table-cell">{type.products_count}</td>
                ...
                <td className="table-cell">
                  <div className="flex gap-2">
                    {hasPermission('references.product_types.edit') && (
                      <Link to={`/references/product-types/${type.id}`} className="text-blue-600 hover:text-blue-800">
                        <Edit className="w-4 h-4" />
                      </Link>
                    )}
                    {hasPermission('references.product_types.delete') && (
        ...
        {filteredTypes.length === 0 && (
          <div className="text-center py-8 text-gray-500">Типы товаров не найдены</div>
        )}
      </div>
    </div>
  );
