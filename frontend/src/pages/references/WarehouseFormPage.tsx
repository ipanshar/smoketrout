...
export default function WarehouseFormPage() {
  ...
  const loadWarehouse = async () => {
    try {
      ...
      navigate('/references/warehouses');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/references/warehouses/${id}`, formData);
      } else {
        await api.post('/references/warehouses', formData);
      }
      navigate('/references/warehouses');
    } catch (error: any) {
    ...
    <div className="p-6 max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-4">
        ...
        <div className="flex gap-3 pt-4">
          ...
          <button type="button" onClick={() => navigate('/references/warehouses')} className="btn btn-secondary">
            Отмена
          </button>
        </div>
      </form>
