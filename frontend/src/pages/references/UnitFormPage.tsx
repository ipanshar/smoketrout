...
export default function UnitFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/references/units/${id}`, formData);
      } else {
        await api.post('/references/units', formData);
      }
      navigate('/references/units');
    } catch (error: any) {
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">
        {isEditing ? 'Редактирование единицы измерения' : 'Новая единица измерения'}
      </h1>
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-4">
        ...
        <div className="flex gap-3 pt-4">
          ...
          <button type="button" onClick={() => navigate('/references/units')} className="btn btn-secondary">
            Отмена
          </button>
        </div>
      </form>
