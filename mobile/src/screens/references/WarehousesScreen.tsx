...
export default function WarehousesScreen({navigation}: Props) {
  ...
  const handleDelete = (item: Warehouse) => {
    Alert.alert('Удалить?', `Удалить "${item.name}"?`, [
      ...
      {text: 'Удалить', style: 'destructive', onPress: async () => {
        try {
          await api.delete(`/references/warehouses/${item.id}`);
  ...
  const renderItem = ({item}: {item: Warehouse}) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => navigation.navigate('WarehouseForm', {id: item.id})}
      onLongPress={() => hasPermission('references.warehouses.delete') && handleDelete(item)}>
