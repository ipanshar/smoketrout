...
export default function WarehouseFormScreen({navigation, route}: Props) {
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      const payload = {name, code, address, description, is_active: isActive};
      if (isEditing) await api.put(`/references/warehouses/${id}`, payload);
      else await api.post('/references/warehouses', payload);
      navigation.goBack();
    } catch (error: any) {
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.form}>
        ...
        <TextInput style={styles.input} value={name} onChangeText={setName} placeholder="Основной склад" />
