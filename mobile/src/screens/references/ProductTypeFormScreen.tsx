...
export default function ProductTypeFormScreen({navigation, route}: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing) await api.put(`/references/product-types/${id}`, payload);
      else await api.post('/references/product-types', payload);
      navigation.goBack();
    } catch (error: any) {
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.form}>
        ...
        <TextInput style={styles.input} value={name} onChangeText={setName} placeholder="Рыба" />

        <Text style={styles.label}>Код *</Text>
        <TextInput style={styles.input} value={code} onChangeText={setCode} placeholder="fish" editable={!isEditing} />
