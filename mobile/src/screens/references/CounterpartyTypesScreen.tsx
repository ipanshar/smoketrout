...
export default function CounterpartyTypesScreen({navigation}: Props) {
  const {hasPermission} = useAuth();
  const [items, setItems] = useState<CounterpartyType[]>([]);
  ...
  const handleDelete = (item: CounterpartyType) => {
    Alert.alert('Удалить?', `Удалить "${item.name}"?`, [
      {text: 'Отмена', style: 'cancel'},
      {text: 'Удалить', style: 'destructive', onPress: async () => {
        try {
          await api.delete(`/references/counterparty-types/${item.id}`);
  ...
  const renderItem = ({item}: {item: CounterpartyType}) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => navigation.navigate('CounterpartyTypeForm', {id: item.id})}
      onLongPress={() => hasPermission('references.counterparty_types.delete') && handleDelete(item)}>
  ...
  return (
    <View style={styles.container}>
      ...
      {hasPermission('references.counterparty_types.create') && (
        <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('CounterpartyTypeForm', {})}>
